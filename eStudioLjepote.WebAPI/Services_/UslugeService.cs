...
namespace eStudioLjepote.WebAPI.Services_
{
    public class UslugeService : IUslugeService
    {
        ...
        public List<Usluga> Get(UslugaSearchRequest request)
        {
            var query = context.Set<Usluge>().AsQueryable();

            if (request?.VrstaUslugeId.HasValue == true)
            {
                query = query.Where(x => x.VrstaUslugeId == request.VrstaUslugeId);
            }
